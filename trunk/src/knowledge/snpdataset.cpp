...
void SnpDataset::AddSNPs(const SnpArray& snps) {
	SnpArray::const_iterator itr = snps.begin();
	SnpArray::const_iterator end = snps.end();
...
uint SnpDataset::LoadData(const SnpArray& data, uint uscBuildVersion) {
	// We haven't linked up to liftover yet
	assert(uscBuildVersion==37);

	std::set<SNP> observedSNPs;		///< To avoid duplicates

	SnpArray::const_iterator itr = data.begin();
	SnpArray::const_iterator end = data.end();
...
SnpDataset::SNP& SnpDataset::operator[](uint index) {
...
uint SnpDataset::ReconcileLiftover(std::multimap<SNP, SNP>& converted, std::ostream& os, int tolerance) {
	
	os<<"Tolerant SNPs:\n"
	  <<"Chrom\tPos\tRS\n";
	SnpArray	notFound									= markers;
	...
	while (file.good()) {
		...
		if (!file.eof()) {
			...
			SnpArray::iterator saItr = notFound.begin();
			SnpArray::iterator saEnd = notFound.end();
			os<<"\nMissing SNPs:\n"
				 <<"Chrom]\tPos\tRS\n";
