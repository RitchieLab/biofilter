...
class DTPBase(DBTuningMixin):
    ...
    def get_path(self, path: str) -> Path:
        raw_path_ds = (
            Path(path) / self.data_source.source_system.name / self.data_source.name  # noqa E501
        )  # noqa: E501
    ...
    def check_compatibility(self):
        ...
        if not metadata:
            raise Exception("❌ Database metadata not found. Schema may not be initialized.")  # noqa E501

        db_version = metadata.schema_version
        db_v = version.parse(db_version)
        min_v = version.parse(self.compatible_schema_min)
        max_v = version.parse(self.compatible_schema_max) if self.compatible_schema_max else None  # noqa E501
