...
class DBTuningMixin:
    ...
    def db_write_mode(self):
        ...
        self.logger.log("⚙️ Applying SQLite PRAGMA optimizations for bulk insert", "DEBUG")  # noqa E501
        ...
        self.session.execute(text("PRAGMA cache_size = -100000;"))  # ~100MB of memory cache  # noqa E501
        self.session.execute(text("PRAGMA foreign_keys = OFF;"))    # Temporarily disable FK checks  # noqa E501
        self.session.commit()
    ...
    def db_read_mode(self):
        ...
        self.logger.log("🔄 Resetting SQLite PRAGMAs to default settings", "DEBUG")  # noqa E501
    ...
    def create_indexes(self, index_specs: list[tuple[str, list[str]]]):
        ...
        if engine not in ("sqlite", "postgresql"):
            self.logger.log(f"❌ Index creation not supported for engine: {engine}", "WARNING")  # noqa E501
            return
    ...
    def drop_indexes(self, index_specs: list[tuple[str, list[str]]]):
        ...
        engine = self.session.bind.dialect.name
        if engine not in ("sqlite", "postgresql"):
            self.logger.log(f"❌ Index removal not supported for engine: {engine}", "WARNING")  # noqa E501
            return
