...
from biofilter.modules.report_legacy.reports.base_report import ReportBase
...
# """
    ...
    def run(self) -> pd.DataFrame:
        ...
        def is_good_extract(r):
            return (r["operation_type"] == "extract") and (
                r["extract_status"] in GOOD_EXTRACT
            )

        def is_good_transform(r):
            return (r["operation_type"] == "transform") and (
                r["transform_status"] in GOOD_STEP
            )
        ...
        df = df.sort_values(
            ["data_source_id", "created_at", "etl_package_id"],
        ...
        for ds_id, g in df.groupby("data_source_id", sort=False):
            ...
            extract_hash = (
                latest_extract["extract_hash"] if latest_extract is not None else None
            )
            transform_hash = (
                latest_transform["transform_hash"]
                if latest_transform is not None
                else None
            )
            load_hash = latest_load["load_hash"] if latest_load is not None else None

            transform_aligned = (extract_hash is not None) and (
                transform_hash == extract_hash
            )
            load_aligned = (transform_hash is not None) and (
                load_hash == transform_hash
            )
            ...
            out_rows.append(
                {
                    ...
                    "extract_package_id": (
                        ...
                        else int(latest_extract["etl_package_id"])
                    ),
                    "extract_status": (
                        None
                        if latest_extract is None
                        else latest_extract["extract_status"]
                    ),
                    "extract_end": (
                        None
                        if latest_extract is None
                        else latest_extract["extract_end"]
                    ),
                    ...
                    "transform_package_id": (
                        ...
                        else int(latest_transform["etl_package_id"])
                    ),
                    "transform_status": (
                        None
                        if latest_transform is None
                        else latest_transform["transform_status"]
                    ),
                    "transform_end": (
                        None
                        if latest_transform is None
                        else latest_transform["transform_end"]
                    ),
                    ...
                    "load_package_id": (
                        ...
                        else int(latest_load["etl_package_id"])
                    ),
                    "load_status": (
                        None if latest_load is None else latest_load["load_status"]
                    ),
                    "load_end": (
                        None if latest_load is None else latest_load["load_end"]
                    ),
                    "load_hash": load_hash,
                    "load_aligned_with_latest_transform": load_aligned,
                    # Convenience: overall “good pipeline?”
                    "pipeline_ok": bool(
                        latest_extract is not None
                        and transform_aligned
                        and load_aligned
                    ),
