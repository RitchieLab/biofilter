...
class AnnotationMasterChemicalReport(ReportBase):
    name = "annotation_master_chemical"
    description = (
        "Compact chemical annotation report with ChemicalMaster identity/properties, "
        "source/provenance, optional xref summary, and optional relationship "
        "summary by entity group."
    )
    ...
    columns = [
        ...
        "chemical_charge",
        "chemical_mass",
        "chemical_monoisotopic_mass",
        "chemical_structure_id",
        "chemical_is_autogenerated",
        "omic_status",
        "chemical_source_system",
        "chemical_data_source",
    ...
    def _resolve_all_chemical_entities(
        self,
        ...
        rows = (
            self.session.query(
                ...
                ChemicalMaster.chemical_id.label("chemical_id"),
                ChemicalMaster.name.label("chemical_name"),
                primary_alias.alias_value.label("primary_name"),
            )
            .join(ChemicalMaster, ChemicalMaster.entity_id == Entity.id)
            ...
            .filter(
                func.lower(EntityGroup.name).in_(
                    ["chemical", "chemicals", "compound", "compounds"]
                )
            )
            .order_by(Entity.id)
            .all()
        ...
        for r in rows:
            ...
            display_alias = (
                ...
                or _norm(r.chemical_id)
                or _norm(r.chemical_name)
    ...
    def _resolve_input_entities(
        ...
        for key in list(best_by_key.keys()):
            best_by_key[key].pop("_score", None)

        entity_ids = sorted({v["entity_id"] for v in best_by_key.values()})
        return best_by_key, entity_ids

    def _fetch_chemical_core(self, entity_ids: list[int]) -> dict[int, dict[str, Any]]:
        if not entity_ids:
            return {}

        rows = (
            self.session.query(
                ChemicalMaster.entity_id.label("entity_id"),
                ChemicalMaster.id.label("chemical_master_id"),
                ChemicalMaster.chemical_id.label("chemical_id"),
                ChemicalMaster.name.label("chemical_name"),
                ChemicalMaster.definition.label("chemical_definition"),
                ChemicalMaster.formula.label("chemical_formula"),
                ChemicalMaster.charge.label("chemical_charge"),
                ChemicalMaster.mass.label("chemical_mass"),
                ChemicalMaster.monoisotopic_mass.label("chemical_monoisotopic_mass"),
                ChemicalMaster.structure_id.label("chemical_structure_id"),
                ChemicalMaster.is_autogenerated.label("chemical_is_autogenerated"),
                ChemicalMaster.etl_package_id.label("chemical_etl_package_id"),
                OmicStatus.name.label("omic_status"),
                ETLDataSource.name.label("chemical_data_source"),
                ETLSourceSystem.name.label("chemical_source_system"),
            )
            .join(
                OmicStatus,
                OmicStatus.id == ChemicalMaster.omic_status_id,
                isouter=True,
            )
            .join(
                ETLDataSource,
                ETLDataSource.id == ChemicalMaster.data_source_id,
                isouter=True,
            )
            .join(
                ETLSourceSystem,
            ...
            .filter(ChemicalMaster.entity_id.in_(entity_ids))
            .order_by(ChemicalMaster.entity_id, ChemicalMaster.id)
            .all()
        )
        ...
        for r in rows:
            ...
            if eid in out:
                continue
            out[eid] = {
                "chemical_master_id": int(r.chemical_master_id),
                "chemical_id": r.chemical_id,
                "chemical_name": r.chemical_name,
                "chemical_definition": r.chemical_definition,
                "chemical_formula": r.chemical_formula,
                "chemical_charge": r.chemical_charge,
                "chemical_mass": r.chemical_mass,
                "chemical_monoisotopic_mass": r.chemical_monoisotopic_mass,
                "chemical_structure_id": r.chemical_structure_id,
                "chemical_is_autogenerated": r.chemical_is_autogenerated,
                "omic_status": r.omic_status,
                "chemical_data_source": r.chemical_data_source,
                "chemical_source_system": r.chemical_source_system,
                "chemical_etl_package_id": r.chemical_etl_package_id,
            }
        return out
    ...
    def run(self):
        ...
        if all_mode:
            input_entries, resolved_by_key, entity_ids = (
                self._resolve_all_chemical_entities()
            )
            if not entity_ids:
                raise ValueError(
                    "No chemical entities found in database for input_data='__ALL__'."
                )
        else:
        ...
        chemical_core_by_entity = self._fetch_chemical_core(entity_ids)
        ...
        for entry in input_entries:
                ...
                records.append(
                    {
                        ...
                        "entity_id": None,
                        "chemical_master_id": None,
                        "chemical_id": None,
                        "chemical_name": None,
                        "chemical_definition": None,
                        "chemical_formula": None,
                        "chemical_charge": None,
                        "chemical_mass": None,
                        "chemical_monoisotopic_mass": None,
                        "chemical_structure_id": None,
                        "chemical_is_autogenerated": None,
                        "omic_status": None,
                        "chemical_source_system": None,
                        "chemical_data_source": None,
                        "chemical_etl_package_id": None,
                        ...
                        "status": "not_found",
                        "note": "Input not resolved to a Chemical entity.",
                    }
                )
                continue

            entity_id = int(resolved["entity_id"])
            core = chemical_core_by_entity.get(entity_id, {})
            ...
            if not core:
                status = "partial"
                notes.append("Chemical entity resolved but no ChemicalMaster row found.")

            if include_aliases:
                canonical_values = {
                    _norm(core.get("chemical_id")),
                    _norm(core.get("chemical_name")),
                ...
                other_aliases = sorted(
                    {
                        _norm(a.alias_value)
                        for a in aliases
                        if _norm(a.alias_value)
                        and _norm(a.alias_value) not in canonical_values
                    }
                )
            else:
                other_aliases = None
            ...
            records.append(
                {
                    ...
                    "entity_id": entity_id,
                    "chemical_master_id": core.get("chemical_master_id"),
                    "chemical_id": core.get("chemical_id"),
                    "chemical_name": core.get("chemical_name"),
                    "chemical_definition": core.get("chemical_definition"),
                    "chemical_formula": core.get("chemical_formula"),
                    "chemical_charge": core.get("chemical_charge"),
                    "chemical_mass": core.get("chemical_mass"),
                    "chemical_monoisotopic_mass": core.get("chemical_monoisotopic_mass"),
                    "chemical_structure_id": core.get("chemical_structure_id"),
                    "chemical_is_autogenerated": core.get("chemical_is_autogenerated"),
                    "omic_status": core.get("omic_status"),
                    "chemical_source_system": core.get("chemical_source_system"),
                    "chemical_data_source": core.get("chemical_data_source"),
                    "chemical_etl_package_id": core.get("chemical_etl_package_id"),
                    "xref_ids_by_source": (
