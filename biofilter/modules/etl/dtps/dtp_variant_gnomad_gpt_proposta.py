...
class DTP(DTPBase):
        ...
        conn.execute(
            text(
                ...
                CREATE TEMP TABLE IF NOT EXISTS tmp_gnomad_consequence_stage (
                    chromosome integer NULL,
                    variant_key varchar(256) NOT NULL,
                    gene_id_raw varchar(32) NULL,
                    gene_symbol_raw varchar(64) NULL,
                    gene_id varchar(32) NULL,
                    gene_symbol varchar(64) NULL,
                    transcript_id varchar(32) NULL,
                    transcript_id_raw varchar(32) NULL,
                    feature_type varchar(32) NULL,
                    consequence_raw varchar(255) NULL,
                    consequence varchar(64) NULL,
                    consequence_group varchar(64) NULL,
                    consequence_category varchar(64) NULL,
                    consequence_rank integer NULL,
                    impact varchar(16) NULL,
                    impact_rank integer NULL,
                    biotype varchar(64) NULL,
                    variant_class varchar(16) NULL,
                    ...
                    lof_info text NULL,
                    most_severe_consequence_per_annotation varchar(64) NULL,
                    most_severe_consequence_per_variant varchar(64) NULL,
    ...
    def _bulk_insert_variant_molecular_effects_from_stage(self, conn) -> int:
        ...
        column_expr_map = {
            "chromosome": "vm.chromosome",
            "variant_id": "vm.variant_id",
            "variant_key": "c.variant_key",
            "gene_id": "COALESCE(c.gene_id_raw, c.gene_id)",
            "gene_symbol": "COALESCE(c.gene_symbol_raw, c.gene_symbol)",
            "transcript_id": "COALESCE(c.transcript_id, c.transcript_id_raw)",
            "feature_type": "c.feature_type",
            "consequence_raw": "c.consequence_raw",
            "consequence_id": "vc.id",
            "impact_id": "vi.id",
            "biotype_id": "vb.id",
            "consequence_group_id": "vcg.id",
            "consequence_category_id": "vcc.id",
            "consequence_rank": "c.consequence_rank",
            "impact_rank": "c.impact_rank",
            "most_severe_consequence_per_annotation_id": "vc_annot.id",
            "most_severe_consequence_per_variant_id": "vc_var.id",
            "is_most_severe_for_annotation": "c.is_most_severe_for_annotation",
            "is_most_severe_for_variant": "c.is_most_severe_for_variant",
            "variant_class": "c.variant_class",
            "canonical": "c.canonical",
            "mane_select": "c.mane_select",
            "mane_plus_clinical": "c.mane_plus_clinical",
            "hgvsc": "c.hgvsc",
            "hgvsp": "c.hgvsp",
            "cdna_position": "c.cdna_position",
            "cds_position": "c.cds_position",
            "protein_position": "c.protein_position",
            "amino_acids": "c.amino_acids",
            "codons": "c.codons",
            "ensp": "c.ensp",
            "lof_flag": "c.lof_flag",
            "lof_confidence": "c.lof_confidence",
            "lof_filter": "c.lof_filter",
            "lof_flags": "c.lof_flags",
            "lof_info": "c.lof_info",
            "data_source_id": ":data_source_id",
            "etl_package_id": ":etl_package_id",
        }

        for col_name in column_expr_map:
            if col_name in vme.c:
                insert_columns.append(col_name)
                select_expressions.append(column_expr_map[col_name])

        if not insert_columns:
            return 0

        conflict_cols = ["chromosome", "variant_id", "transcript_id", "consequence_id"]
        if any(col not in insert_columns for col in conflict_cols):
            raise RuntimeError(
                "variant_molecular_effects fast load requires the PK columns to exist."
            )
        ...
        sql = f"""
            ...
            FROM tmp_gnomad_consequence_stage c
            JOIN tmp_gnomad_variant_stage s
              ON s.variant_key = c.variant_key
            ...
            JOIN variant_consequences vc
              ON vc.name = c.consequence
            LEFT JOIN variant_impacts vi
              ON vi.name = c.impact
            LEFT JOIN variant_biotypes vb
              ON vb.name = c.biotype
            LEFT JOIN variant_consequence_groups vcg
              ON vcg.name = c.consequence_group
            LEFT JOIN variant_consequence_categories vcc
              ON vcc.name = c.consequence_category
            LEFT JOIN variant_consequences vc_annot
              ON vc_annot.name = c.most_severe_consequence_per_annotation
            LEFT JOIN variant_consequences vc_var
              ON vc_var.name = c.most_severe_consequence_per_variant
            WHERE COALESCE(c.transcript_id, c.transcript_id_raw) IS NOT NULL
              AND c.consequence IS NOT NULL
            ON CONFLICT (
                chromosome,
                variant_id,
                transcript_id,
    ...
    def _upsert_variant_molecular_effects_from_df(
        ...
        for row in df.itertuples(index=False):
            ...
            record = {
                ...
                "consequence_group_id": self._none_if_nan(
                    getattr(row, "consequence_group_id", None)
                ),
                "consequence_category_id": self._none_if_nan(
                    getattr(row, "consequence_category_id", None)
                ),
                "consequence_raw": getattr(row, "consequence_raw", None),
                "consequence_rank": getattr(row, "consequence_rank", None),
                "impact_rank": getattr(row, "impact_rank", None),
                "most_severe_consequence_per_annotation_id": None,
                "most_severe_consequence_per_variant_id": None,
                "is_most_severe_for_annotation": getattr(
                    row, "is_most_severe_for_annotation", None
                ),
                "is_most_severe_for_variant": getattr(
                    row, "is_most_severe_for_variant", None
                ),
    ...
    def _load_postgres_part_file_fast(
        ...
        variant_stage_df = df_variants.copy()
        variant_stage_df["ac"] = (
            variant_stage_df["AC"] if "AC" in variant_stage_df.columns else None
        )
        variant_stage_df["an"] = (
            variant_stage_df["AN"] if "AN" in variant_stage_df.columns else None
        )
        variant_stage_df["af"] = (
            variant_stage_df["AF"] if "AF" in variant_stage_df.columns else None
        )
        ...
        for col in variant_stage_columns:
            if col not in variant_stage_df.columns:
                variant_stage_df[col] = None
        ...
        resolved_variant_ids = conn.execute(
            text(
                """
                SELECT COUNT(DISTINCT s.variant_key)
                FROM tmp_gnomad_variant_stage s
                JOIN variant_masters vm
                  ON vm.chromosome = s.chromosome
                 AND vm.position_start = s.position_start
        ...
        if df_consequences is not None and not df_consequences.empty:
            ...
            for col in consequence_stage_columns:
                if col not in df_consequences.columns:
                    df_consequences[col] = None

            self._copy_dataframe_to_postgres_stage(
                conn,
                table_name="tmp_gnomad_consequence_stage",
                df=df_consequences,
                columns=consequence_stage_columns,
            )
            loaded_effects = self._bulk_insert_variant_molecular_effects_from_stage(
                conn
            )

        return processed_variant_rows, int(resolved_variant_ids), loaded_effects
    ...
    def load(self, processed_dir=None):
        ...
        try:
            with self.db.engine.begin() as conn:
                ...
                use_fast_load = self._supports_postgres_fast_load(conn)

                load_chrom = None
                if use_fast_load:
                    load_chrom = resolve_file_chromosome(
                        Path(self.data_source.name), self.data_source.name
                    )
                    if load_chrom is None:
                        raise ValueError(
                            "Could not resolve chromosome for partition refresh."
                        )

                    self._create_postgres_stage_tables(conn)
                    if getattr(self.config, "postgres_partition_refresh", True):
                    ...
                    self.logger.log(
                        "🚀 Using PostgreSQL fast load with temp staging tables.",
                        "INFO",
                    )
                ...
                for variant_file in variant_files:
                    ...
                    if use_fast_load:
                        ...
                        processed_variant_rows = self._upsert_variant_masters_from_df(
                            df_variants,
                            conn,
                        )
                        variant_id_map = self._resolve_variant_ids_for_df(
                            df_variants, conn
                        )
                        resolved_variant_ids = len(variant_id_map)
                        loaded_effects = 0
                        if (
                            df_consequences is not None
                            and not df_consequences.empty
                        ):
                            loaded_effects = (
                                self._upsert_variant_molecular_effects_from_df(
                                    df_consequences,
                                    variant_id_map,
                                    conn,
