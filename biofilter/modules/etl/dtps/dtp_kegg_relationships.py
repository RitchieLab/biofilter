...
# Maps KEGG relation_type -> EntityRelationshipType.code
...
class DTP(DTPBase, EntityQueryMixin):
    def __init__(
        ...
        # DTP versioning
        self.dtp_name = "dtp_kegg_relationships"
    ...
    def extract(self, raw_dir: str):
        """
        No extraction. Uses the parent 'kegg_pathways' DTP processed output.
        """
        msg = (
            f"⚠️  '{self.data_source.name}' is for relationships only. "
            "Use 'kegg_pathways' to extract raw data."
        )
    ...
    def transform(self, raw_dir: str, processed_dir: str):
        """
        No transformation. Uses the parent 'kegg_pathways' processed output.
        """
        msg = (
            f"⚠️  '{self.data_source.name}' is for relationships only. "
            "Transformation goes through the 'kegg_pathways' data source."
        )
        self.logger.log(msg, "INFO")
        return True, msg
    ...
    def load(self, processed_dir=None):
        """
        Load KEGG gene <-> pathway memberships into entity_relationships
        using a temp table + INSERT...SELECT...NOT EXISTS for performance.

        Mirrors dtp_reactome_relationships:
        ...
        # ------------------------------------------------------------------
        # 3) Resolve relationship_type IDs (in_pathway)
        # ------------------------------------------------------------------
        try:
            ...
            if missing:
                msg = f"⚠️  Relationship types not found in DB: {missing}"
                self.logger.log(msg, "ERROR")
                return False, msg
        except Exception as e:
        ...
        # ------------------------------------------------------------------
        # 4) Resolve entity_1 (always Pathway via KEGG pathway_id)
        # ------------------------------------------------------------------
        try:
            pathway_ids = (
                df["pathway_id"].dropna().astype(str).unique().tolist()
            )
            ...
            df["entity_1_id"] = df["pathway_id"].map(pathway_map)
        except Exception as e:
        ...
        # ------------------------------------------------------------------
        # 5) Resolve entity_2 (Gene via NCBI/Entrez gene id)
        # ------------------------------------------------------------------
        try:
            df["entity_2_id"] = pd.NA

            mask = df["relation_type"] == "ncbi_gene"
            if mask.any():
                vals = (
                ...
                gene_map = self._resolve_aliases(
                    values=vals,
                ...
                df.loc[mask, "entity_2_id"] = (
                    df.loc[mask, "relation"].map(gene_map)
                )
        except Exception as e:
        ...
        if not df_invalid.empty:
            invalid_path = processed_path / "kegg_rel_not_loaded.csv"
    ...
    def _bulk_insert_relationships(self, df_valid):
        ...
        """
        temp_table = "kegg_load_candidates"
        total_inserted = 0
        insert_error = None
        ...
        msg = (
            f"📥 Total KEGG relationships inserted: {total_inserted:,}"
        )
        return True, msg
