...
class GenomeAssembly(Base):
    ...
    variants = relationship("Variant", back_populates="assembly", cascade="all, delete-orphan")
...
class Variant(Base):
    ...
    rs_id = Column(String, primary_key=True)  # rsID como chave primária
    position = Column(Integer, nullable=False)
    assembly_id = Column(Integer, ForeignKey("genome_assemblies.id"), nullable=False)
    chromosome = Column(String)  # TEXT para suportar X, Y, MT etc.
    ref = Column(Text)
    alt = Column(Text)
    data_source_id = Column(Integer, ForeignKey("etl_data_sources.id"), nullable=False)
