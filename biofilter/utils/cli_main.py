...
def auto_register_etl_commands():
    ...
    for name, method in inspect.getmembers(BiofilterClass, predicate=inspect.isfunction):  # noqa: E501
        ...
        def create_cmd(method=method, sig=sig):
            ...
            def command(ctx, **kwargs):
                ...
                if not db_uri:
                    db_uri = get_db_uri_from_config()
                if not db_uri:
                    raise click.UsageError("Missing required --db-uri. Use option or set in .biofilter.toml")  # noqa: E501
...
@report.command("run")
@click.option("--name", required=True, help="Report name (e.g., qry_etl_status)")  # noqa: E501
@click.option("--db-uri", required=False, help="Database URI or use .biofilter.toml")  # noqa: E501
