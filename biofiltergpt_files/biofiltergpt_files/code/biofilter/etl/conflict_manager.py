...
class ConflictManager:
    ...
    def import_conflicts_from_excel(
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if not conflict:
                    self.logger.log(
                        f"⚠️ Conflict ID {conflict_id} not found", "WARNING"
                    )  # noqa E501
                    continue
                ...
                if has_changes:
                    updated += 1
                    self.logger.log(
                        f"🔄 Updated conflict ID {conflict_id}", "DEBUG"
                    )  # noqa E501

            self.session.commit()
            self.logger.log(
                f"✅ {updated} conflict(s) updated from Excel", "INFO"
            )  # noqa E501
            return updated

        except Exception as e:
            self.logger.log(
                f"❌ Failed to import conflicts from Excel: {e}", "ERROR"
            )  # noqa E501
            return 0
    ...
    def apply_resolution(self, row):
        ...
        if resolution == ConflictResolution.delete:
            msg = f"🗑️ Applying DELETE resolution for Gene {hgnc_id}"
            self.logger.log(msg, "INFO")
            ...
            if entity:
                ...
                msg = f"🔒 Entity {entity.id} marked as inactive (delete)"
                self.logger.log(msg, "DEBUG")
            else:
                msg = f"⚠️ Entity ID {conflict.entity_id} not found"
            ...
            if gene:
                ...
                msg = f"⚠️ Gene {hgnc_id} not found during delete resolution"
                self.logger.log(msg, "WARNING")
        ...
        # MERGE BLOCK
            ...
            if not target_gene:
                msg = f"❌ Target gene '{conflict.existing_identifier}' not found for merge"  # noqa: E501
                self.logger.log(msg, "ERROR")
                return False

            # 🔒 Mark the old Entity as inactive
            source_entity = (
                self.session.query(Entity)
                .filter_by(id=conflict.entity_id)
                .first()  # noqa: E501
            ...
            if source_entity:
                ...
                msg = f"🔒 Entity {source_entity.id} marked as inactive (merged)"  # noqa: E501
                self.logger.log(msg, "DEBUG")
            else:
                self.logger.log(
                    f"⚠️ Source entity ID {conflict.entity_id} not found",
                    "WARNING",  # noqa: E501
                )
        ...
        # KEEP_BOTH BLOCk
            ...
            msg = f"⚖️ KEEP_BOTH resolution: {hgnc_id} and {conflict.existing_identifier}"  # noqa: E501
            self.logger.log(msg, "INFO")
            ...
            if entity:
                entity.has_conflict = True
                msg = f"🔒 Entity {entity.id} marked with conflict (keep_both)"
                self.logger.log(msg, "DEBUG")
            else:
                msg = f"⚠️ Entity ID {conflict.entity_id} not found"
                self.logger.log(msg, "WARNING")
            ...
            msg = f"✔️ Both genes '{hgnc_id}' and '{conflict.item_exist}' kept — shared ID resolution will favor '{conflict.item_exist}'"  # noqa: E501
            self.logger.log(msg, "INFO")
