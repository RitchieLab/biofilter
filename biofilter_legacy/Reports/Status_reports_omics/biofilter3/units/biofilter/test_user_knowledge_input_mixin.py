...
class MockLoki:
    def __init__(self):
        ...
        self._biofilter.db.execute("ATTACH DATABASE ':memory:' AS user")
        self._biofilter.db.execute("ATTACH DATABASE ':memory:' AS db")

        # Cria a tabela `source` dentro do esquema `user`
        self._biofilter.db.execute(
...
def test_add_user_source(mixin):
    ...
    assert source_id == -1
    cursor = mixin._loki._biofilter.db.cursor()
    cursor.execute(
...
def test_apply_user_knowledge_filter_gene_level(mixin):
    ...
    # Insere dados simulados nas tabelas user.group_biopolymer e biofilter.db.biopolymer
    cursor.execute(
        "INSERT INTO 'user'.'group_biopolymer' (group_id, biopolymer_id) VALUES (1, 100)"  # noqa: E501
    )
    cursor.execute(
