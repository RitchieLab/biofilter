...
class TestDatabaseManagementMixin:
    ...
    def test_attachDatabaseFile(self, mixin):
        dbFile = "external_biofilter.db.sqlite"
    ...
    def test_tableHasData_with_data(self, mixin):
        db, table = "test_db", "source"
        mixin._loki._biofilter.db.cursor().execute.return_value = [(1,)]
        assert mixin.tableHasData(db, table) is True
        mixin._loki._biofilter.db.cursor().execute.assert_called_once_with(
            "SELECT 1 FROM `test_db`.`source` LIMIT 1"
        )

    def test_tableHasData_without_data(self, mixin):
        db, table = "test_db", "source"
        mixin._loki._biofilter.db.cursor().execute.return_value = []
    ...
    def test_updateRegionZones(self, mixin):
        ...
        mixin._loki._biofilter.db.cursor().execute.side_effect = [
        ...
        mixin._loki._biofilter.db.cursor().executemany = MagicMock()
        ...
        # Check if the method was called with the correct argument
        mixin._loki._biofilter.db.cursor().execute.assert_any_call(
            "UPDATE `test_db`.`region` SET posMin = posMax, posMax = posMin WHERE posMin > posMax"  # noqa: E501
        )
        # TODO: Returns Error
        # mixin._loki._biofilter.db.cursor().executemany.assert_called_once_with(  # noqa: E501
