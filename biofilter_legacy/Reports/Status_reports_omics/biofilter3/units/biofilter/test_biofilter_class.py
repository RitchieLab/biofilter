...
def test_init_no_options(mock_loki_db, tmp_path):
    ...
    mock_loki_biofilter.db.Database.getVersionTuple.return_value = (2, 2, 1, "a", 2)
    mock_loki_biofilter.db.Database.getVersionString.return_value = "2.2.1a2"
...
def test_init_loki_version_check(mock_loki_db):
    ...
    # Set up the expected return value for the `getVersionTuple` method
    mock_loki_biofilter.db.Database.getVersionTuple.return_value = (2, 0, 1, "a", 2)
    mock_loki_biofilter.db.Database.getVersionString.return_value = "2.0.1a2"
...
def test_database_initialization(mock_loki_db, mock_options, tmp_path):
    ...
    # Set up the expected return value for the `getVersionTuple` method
    mock_loki_biofilter.db.Database = MagicMock()
    mock_loki_biofilter.db.Database.getVersionTuple.return_value = (2, 2, 1, "a", 2)
    mock_loki_biofilter.db.Database.getVersionString.return_value = "2.2.1a2"
    ...
    mock_loki_biofilter.db.Database().attachTempDatabase.assert_called()
    # Check if the database tables were created
    mock_loki_biofilter.db.Database().createDatabaseTables.assert_called()
    ...
    # Verify that _loki was set as an instance of `loki_biofilter.db.Database`
    # mock_loki_biofilter.db.Database.assert_called_once()  # DB was instantiated once
    # assert (
    #     biofilter._loki is mock_loki_biofilter.db.Database()
    # ), "Expected _loki to reference the mocked Database instance."

    # Chack if the `Database` class was instantiated exactly 3 times
    assert mock_loki_biofilter.db.Database.call_count == 3
