...
from biofilter.modules.report_legacy.reports.report_snp_snp_model import SNPSNPModelReport
...
def _create_variant_masters_table(engine):
    ...
    # Dropped first, then created from this definition.
    #
    # `Base.metadata.create_all()` runs before this in the fixture, and
    # `variant_masters` is now declared there — so without the drop the
    # fixture silently inherits the model's shape and its own definition
    # is ignored. That is how this test used to pass: the model and the
    # fixture agreed. They no longer do, because the model was corrected
    # to describe the parquet the build writes (`position`, `variant_key`)
    # while this report still reads the 4.2.x shape (`position_start`,
    # `position_end`, `variant_id`, `allele_type`). The report is one of
    # those ADR-004 replaces; until then the fixture states outright
    # which schema it is testing against.
    vm.drop(engine, checkfirst=True)
