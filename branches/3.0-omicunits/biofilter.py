...
""",
				'index': {
					'group_unit__unit': '(unit_id,group_id)',
				}
			}, #user.group_unit
...
""",
	...
	def __init__(self, options=None):
		...
		minLoki = (3,0,0,'a',1) # 3.0 omic units
	...
	def generateGeneNameStats(self):
		utypeID = self._loki.getUTypeID('gene')
		if not utypeID:
			sys.exit("ERROR: knowledge file contains no gene data")
		return self._loki.generateUnitNameStats(utypeID=utypeID)
	...
	def getOptionUTypeID(self, value, optional=False):
		utypeID = self._loki.getUTypeID(value)
		if not (utypeID or optional):
				...
				for line in file:
					...
					if not words:
						...
						if ugroupID and namesets:
							self.addUserGroupUnits(ugroupID, namesets, errorCallback)
	...
	def unionInputGenes(self, db, names, errorCallback=None):
		...
		self.prepareTableForUpdate(db, 'unit')
		sql = "INSERT INTO `%s`.`unit` (label,extra,unit_id) VALUES (?2,?3,?4); SELECT 1" % db
		maxMatch = (None if self._options.allow_ambiguous_genes == 'yes' else 1)
		tally = dict()
		numAdd = 0
		for row in cursor.executemany(sql, self._loki.generateTypedUnitIDsByIdentifiers(
				self.getOptionUTypeID('gene'), names, minMatch=1, maxMatch=maxMatch, tally=tally, errorCallback=errorCallback
		)):
			numAdd += 1
		if tally['zero']:
		...
		self._inputFilters[db]['unit'] += 1
	#unionInputGenes()
	
	
	def intersectInputGenes(self, db, names, errorCallback=None):
		# names=[ (namespace,name), ... ]
		if not self._inputFilters[db]['unit']:
			return self.unionInputGenes(db, names, errorCallback)
		self.logPush("reducing %s gene filter ...\n" % db)
		cursor = self._loki._db.cursor()
		
		self.prepareTableForQuery(db, 'unit')
		cursor.execute("UPDATE `%s`.`unit` SET flag = 0" % db)
		numBefore = cursor.getconnection().changes()
		tally = dict()
		sql = "UPDATE `%s`.`unit` SET flag = 1 WHERE unit_id = ?4" % db
		maxMatch = (None if self._options.allow_ambiguous_genes == 'yes' else 1)
		cursor.executemany(sql, self._loki.generateTypedUnitIDsByIdentifiers(
				self.getOptionUTypeID('gene'), names, minMatch=1, maxMatch=maxMatch, tally=tally, errorCallback=errorCallback
		))
		cursor.execute("DELETE FROM `%s`.`unit` WHERE flag = 0" % db)
		numDrop = cursor.getconnection().changes()
		if tally['zero']:
			self.warn("WARNING: ignored %d unrecognized gene identifier(s)\n" % tally['zero'])
		if tally['many']:
	...
	def unionInputGeneSearch(self, db, texts):
		...
		utypeID = self.getOptionUTypeID('gene')
		
		self.prepareTableForUpdate(db, 'unit')
		sql = "INSERT INTO `%s`.`unit` (extra,label,unit_id) VALUES (?1,?2,?3); SELECT 1" % db
		numAdd = 0
		for row in cursor.executemany(sql, self._loki.generateTypedUnitIDsBySearch(utypeID, texts)):
			numAdd += 1
		self.logPop("... OK: added %d genes\n" % numAdd)
	...
	def intersectInputGeneSearch(self, db, texts):
		...
		cursor.execute("UPDATE `%s`.`unit` SET flag = 0" % db)
		numBefore = cursor.getconnection().changes()
		sql = "UPDATE `%s`.`unit` SET flag = 1 WHERE unit_id = ?3" % db
		cursor.executemany(sql, self._loki.generateTypedUnitIDsBySearch(utypeID, texts))
		cursor.execute("DELETE FROM `%s`.`unit` WHERE flag = 0" % db)
		numDrop = cursor.getconnection().changes()
		self.logPop("... OK: kept %d genes (%d dropped)\n" % (numBefore-numDrop,numDrop))
	...
	def addUserGroupUnits(self, ugroupID, namesets, errorCallback=None):
		#TODO: apply ambiguity settings and heuristics?
		# namesets=[ [ (ns,name,extra), ...], ... ]
		self.logPush("adding genes to user-defined group ...\n")
		cursor = self._loki._db.cursor()
		
		sql = "INSERT OR IGNORE INTO `user`.`group_unit` (group_id,unit_id) VALUES (%d,?4)" % (ugroupID,)
		tally = dict()
		cursor.executemany(sql,
			self._loki.generateTypedUnitIDsByIdentifiers(
				self.getOptionUTypeID('gene'),
		...
		if tally['many']:
			self.warn("WARNING: added multiple results for %d ambiguous gene identifier(s)\n" % tally['many'])
		numAdd = sum(row[0] for row in cursor.execute("SELECT COUNT() FROM `user`.`group_unit` WHERE group_id = ?", (ugroupID,)))
		
		self.logPop("... OK: added %d genes\n" % numAdd)
		self._inputFilters['user']['group_units'] += 1
	#addUserGroupUnits()
...
FROM `user`.`group_unit` AS u_gu
JOIN `db`.`group_unit` AS d_gu
  ON d_gu.unit_id = u_gu.unit_id
JOIN `db`.`group` AS d_g
  ON d_g.group_id = d_gu.group_id
"""
			cursor.execute(sql)
			...
			sql = """
INSERT INTO `main`.`gene` (label,unit_id,extra)
SELECT DISTINCT d_u.label, d_u.unit_id, NULL AS extra
FROM `user`.`group_unit` AS u_gu
JOIN `db`.`unit` AS d_u
  ON d_u.unit_id = u_gu.unit_id
"""
			cursor.execute(sql)
	...
	def generatePARISResults(self, ucscBuildUser, ucscBuildDB):
		...
		queryGroupSelect = ['group_id','group_label','group_description','unit_id','unit_label','unit_description']
	...
	_queryAliasTable = {
		...
		'a_u'    : ('alt','unit'),              # (label,unit_id)
		'a_g'    : ('alt','group'),             # (label,group_id)
		'a_c'    : ('alt','source'),            # (label,source_id)
		'c_mu_L' : ('cand','main_unit'),        # (unit_id)
		'c_mu_R' : ('cand','main_unit'),        # (unit_id)
		'c_au_R' : ('cand','alt_unit'),         # (unit_id)
		'c_g'    : ('cand','group'),            # (group_id)
		'u_gu'   : ('user','group_unit'),       # (group_id,unit_id)
		'u_gu_L' : ('user','group_unit'),       # (group_id,unit_id)
		'u_gu_R' : ('user','group_unit'),       # (group_id,unit_id)
		'u_g'    : ('user','group'),            # (group_id,source_id)
		'u_c'    : ('user','source'),           # (source_id)
		'd_sl'   : ('db','snp_locus'),          # (rs,chr,pos)
		'd_r'    : ('db','region'),             # (region_id,rtype_id,chr,posMin,posMax)
		'd_rz'   : ('db','region_zone'),        # (region_id,rtype_id,chr,zone)
		'd_ur'   : ('db','unit_region'),        # (unit_id,region_id,urtype_id)
		'd_u'    : ('db','unit'),               # (unit_id,utype_id,label)
		'd_gu'   : ('db','group_unit'),         # (group_id,unit_id,specificity,implication,quality)
		'd_gu_L' : ('db','group_unit'),         # (group_id,unit_id,specificity,implication,quality)
		'd_gu_R' : ('db','group_unit'),         # (group_id,unit_id,specificity,implication,quality)
		'd_g'    : ('db','group'),              # (group_id,gtype_id,label,source_id)
	...
	#	}),
		# TODO: ldprofiles
	#	frozenset({'d_r'}) : frozenset({
	#		"{L}.ldprofile_id = {ldprofileID}",
	#	}),
		frozenset({'d_gu','d_gu_L','d_gu_R'}) : frozenset({
			"{L}.unit_id != 0",
			"({L}.{guColumn1} {guCondition} OR {L}.{guColumn2} {guCondition})",
		}),
	...
	# The query will then contain redundant conditions, but in exchange
	# the optimizer will always have a suitable condition to apply any
	# available index.
	_queryAliasJoinConditions = {
		...
		(frozenset({'m_l','a_l','d_sl'}),frozenset({'m_rz','a_rz','d_rz'})) : frozenset({
			"{L}.chr = {R}.chr",
		...
		(frozenset({'d_rz'}),frozenset({'d_r'})) : frozenset({
			"{L}.region_id = {R}.region_id",
			# with the rowid match, these should all be guaranteed by loki_updater.updateRegionZones()
			#"{L}.chr = {R}.chr",
			#"(({L}.zone + 1) * {zoneSize}) > {R}.posMin",
			#"({L}.zone * {zoneSize}) <= {R}.posMax",
			#"{L}.zone >= ({R}.posMin / {zoneSize})",
		...
		(frozenset({'m_rz','a_rz','d_rz'}),) : frozenset({
			"{L}.chr = {R}.chr",
		...
		(frozenset({'d_r'}),frozenset({'d_ur'})) : frozenset({
			"{L}.region_id = {R}.region_id",
		}),
		(frozenset({'m_u','a_u','d_ur','d_u'}),) : frozenset({
			"{L}.unit_id = {R}.unit_id",
		}),
		(frozenset({'m_u','a_u','d_u'}),frozenset({'u_gu','d_gu'})) : frozenset({
			"{L}.unit_id = {R}.unit_id",
		}),
		(frozenset({'u_gu_L','u_gu_R'}),) : frozenset({
			"{L}.unit_id != {R}.unit_id",
		}),
		(frozenset({'d_gu_L','d_gu_R'}),) : frozenset({
			"{L}.unit_id != {R}.unit_id",
		}),
		(frozenset({'m_g','a_g','u_gu','d_gu','d_g'}),) : frozenset({
		...
		(frozenset({'c_mu_L'}),frozenset({'u_gu_L','d_gu_L'})) : frozenset({
			"{L}.unit_id = {R}.unit_id",
		}),
		(frozenset({'c_mu_R','c_au_R'}),frozenset({'u_gu_R','d_gu_R'})) : frozenset({
			"{L}.unit_id = {R}.unit_id",
		}),
		(frozenset({'c_g','d_g'}),frozenset({'d_gu','d_gu_L','d_gu_R','d_g'})) : frozenset({
			"{L}.group_id = {R}.group_id",
		}),
		(frozenset({'c_g','u_g'}),frozenset({'u_gu','u_gu_L','u_gu_R','u_g'})) : frozenset({
			"{L}.group_id = {R}.group_id",
		}),
	...
	_queryAliasPairConditions = {
		(frozenset({'m_l','a_l','d_sl'}),frozenset({'m_r','a_r','d_r'})) : frozenset({
			"{L}.chr = {R}.chr",
		...
		(frozenset({'m_r','a_r','d_r'}),) : frozenset({
			"{L}.chr = {R}.chr",
	...
	_queryColumnSources = {
		...
		'region_id' : [
			('a_r', 'rowid',     "a_r.rowid"),
			('m_r', 'rowid',     "m_r.rowid"),
			('d_r', 'region_id', "d_r.region_id"),
		],
		'region_label' : [
			('a_r',  'rowid',   "a_r.label"),
			('m_r',  'rowid',   "m_r.label"),
			('d_u',  'unit_id', "d_u.label"),
		],
		'region_chr' : [ #TODO: find a way to avoid repeating the conversions already in loki_db.chr_name
			...
			('d_r', 'region_id', "(CASE d_r.chr WHEN 23 THEN 'X' WHEN 24 THEN 'Y' WHEN 25 THEN 'XY' WHEN 26 THEN 'MT' ELSE d_r.chr END)"),
		],
		'region_zone' : [
			('a_rz', 'zone', "a_rz.zone"),
			('m_rz', 'zone', "m_rz.zone"),
		...
		'region_start' : [
			...
			('d_r', 'region_id', "d_r.posMin {pMinOffset}"),
		],
		'region_stop' : [
			('a_r', 'rowid',     "a_r.posMax {pMaxOffset}"),
			('m_r', 'rowid',     "m_r.posMax {pMaxOffset}"),
			('d_r', 'region_id', "d_r.posMax {pMaxOffset}"),
		],
		'region_extra' : [
			('a_r', 'rowid',   "a_r.extra"),
			('m_r', 'rowid',   "m_r.extra"),
		...
		'unit_id' : [
			('a_u',    'unit_id', "a_u.unit_id"),
			('m_u',    'unit_id', "m_u.unit_id"),
			('c_mu_L', 'unit_id', "c_mu_L.unit_id"),
			('c_mu_R', 'unit_id', "c_mu_R.unit_id"),
			('c_au_R', 'unit_id', "c_au_R.unit_id"),
			('u_gu',   'unit_id', "u_gu.unit_id"),
			('d_ur',   'unit_id', "d_ur.unit_id"),
			('d_gu',   'unit_id', "d_gu.unit_id"),
			('d_gu_L', 'unit_id', "d_gu_L.unit_id"),
			('d_gu_R', 'unit_id', "d_gu_R.unit_id"),
			('d_u',    'unit_id', "d_u.unit_id"),
		],
		'unit_id_L' : [
			('c_mu_L', 'unit_id', "c_mu_L.unit_id"),
			('u_gu_L', 'unit_id', "u_gu_L.unit_id"),
			('d_gu_L', 'unit_id', "d_gu_L.unit_id"),
			('d_u',    'unit_id', "d_u.unit_id"),
		],
		'unit_id_R' : [
			('c_mu_R', 'unit_id', "c_mu_R.unit_id"),
			('c_au_R', 'unit_id', "c_au_R.unit_id"),
			('u_gu_R', 'unit_id', "d_gu_R.unit_id"),
			('d_gu_R', 'unit_id', "d_gu_R.unit_id"),
			('d_u',    'unit_id', "d_u.unit_id"),
		],
		'unit_label' : [
			('a_u', 'unit_id', "a_u.label"),
			('m_u', 'unit_id', "m_u.label"),
			('d_u', 'unit_id', "d_u.label"),
		],
		'unit_description' : [
			('d_u', 'unit_id', "d_u.description"),
		],
		'unit_identifiers' : [
			('a_u', 'unit_id', "(SELECT GROUP_CONCAT(namespace||':'||name,'|') FROM `db`.`unit_name` AS d_un JOIN `db`.`namespace` AS d_n USING (namespace_id) WHERE d_un.unit_id = a_u.unit_id)"),
			('m_u', 'unit_id', "(SELECT GROUP_CONCAT(namespace||':'||name,'|') FROM `db`.`unit_name` AS d_un JOIN `db`.`namespace` AS d_n USING (namespace_id) WHERE d_un.unit_id = m_u.unit_id)"),
			('d_u', 'unit_id', "(SELECT GROUP_CONCAT(namespace||':'||name,'|') FROM `db`.`unit_name` AS d_un JOIN `db`.`namespace` AS d_n USING (namespace_id) WHERE d_un.unit_id = d_u.unit_id)"),
		],
		'unit_chr' : [ #TODO: find a way to avoid repeating the conversions already in loki_db.chr_name
			('d_r', 'region_id', "(CASE d_r.chr WHEN 23 THEN 'X' WHEN 24 THEN 'Y' WHEN 25 THEN 'XY' WHEN 26 THEN 'MT' ELSE d_r.chr END)"),
		],
		'unit_zone' : [
			('d_rz', 'zone', "d_rz.zone"),
		...
		'unit_extra' : [
			('a_u', 'unit_id', "a_u.extra"),
			('m_u', 'unit_id', "m_u.extra"),
			('d_u', 'unit_id', "NULL"),
		],
		'unit_flag' : [
			('a_u', 'unit_id', "a_u.flag"),
			('m_u', 'unit_id', "m_u.flag"),
			('d_u', 'unit_id', "NULL"),
		],
		...
		'gene_id' : [
			('a_u', 'unit_id', "a_u.unit_id"),
			('m_u', 'unit_id', "m_u.unit_id"),
			('d_u', 'unit_id', "d_u.unit_id", {"d_u.utype_id+0 = {utypeID_gene}"}),
		],
		'gene_label' : [
			('a_u', 'unit_id', "a_u.label"),
			('m_u', 'unit_id', "m_u.label"),
			('d_u', 'unit_id', "d_u.label", {"d_u.utype_id+0 = {utypeID_gene}"}),
		],
		'gene_description' : [
			('d_u', 'unit_id', "d_u.description", {"d_u.utype_id+0 = {utypeID_gene}"}),
		],
		'gene_identifiers' : [
			('a_u', 'unit_id', "(SELECT GROUP_CONCAT(namespace||':'||name,'|') FROM `db`.`unit_name` AS d_un JOIN `db`.`namespace` AS d_n USING (namespace_id) WHERE d_un.unit_id = a_u.unit_id)"),
			('m_u', 'unit_id', "(SELECT GROUP_CONCAT(namespace||':'||name,'|') FROM `db`.`unit_name` AS d_un JOIN `db`.`namespace` AS d_n USING (namespace_id) WHERE d_un.unit_id = m_u.unit_id)"),
			('d_u', 'unit_id', "(SELECT GROUP_CONCAT(namespace||':'||name,'|') FROM `db`.`unit_name` AS d_un JOIN `db`.`namespace` AS d_n USING (namespace_id) WHERE d_un.unit_id = d_u.unit_id)", {"d_u.utype_id+0 = {utypeID_gene}"}),
		],
		'gene_symbols' : [
			('a_u', 'unit_id', "(SELECT GROUP_CONCAT(name,'|') FROM `db`.`unit_name` AS d_un WHERE d_un.unit_id = a_u.unit_id AND d_un.namespace_id = {namespaceID_symbol})"),
			('m_u', 'unit_id', "(SELECT GROUP_CONCAT(name,'|') FROM `db`.`unit_name` AS d_un WHERE d_un.unit_id = m_u.unit_id AND d_un.namespace_id = {namespaceID_symbol})"),
			('d_u', 'unit_id', "(SELECT GROUP_CONCAT(name,'|') FROM `db`.`unit_name` AS d_un WHERE d_un.unit_id = d_u.unit_id AND d_un.namespace_id = {namespaceID_symbol})", {"d_u.utype_id+0 = {utypeID_gene}"}),
		],
		'gene_extra' : [
			('a_u', 'unit_id', "a_u.extra"),
			('m_u', 'unit_id', "m_u.extra"),
			('d_u', 'unit_id', "NULL", {"d_u.utype_id+0 = {utypeID_gene}"}),
		],
		'gene_flag' : [
			('a_u', 'unit_id', "a_u.flag"),
			('m_u', 'unit_id', "m_u.flag"),
		...
		'upstream_id' : [
			('a_l',  'rowid',   "(SELECT d_u.unit_id               FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMax < a_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_u.unit_id               FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMax < m_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_u.unit_id               FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMax < d_sl.pos - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
		],
		'upstream_label' : [
			('a_l',  'rowid',   "(SELECT d_u.label                 FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMax < a_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_u.label                 FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMax < m_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_u.label                 FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMax < d_sl.pos - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
		],
		'upstream_distance' : [
			('a_l',  'rowid',   " a_l.pos -(SELECT MAX(d_r.posMax) FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMax < a_l.pos  - {rpMargin})"),
			('m_l',  'rowid',   " m_l.pos -(SELECT MAX(d_r.posMax) FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMax < m_l.pos  - {rpMargin})"),
			('d_sl', '_ROWID_', " d_sl.pos-(SELECT MAX(d_r.posMax) FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMax < d_sl.pos - {rpMargin})"),
		],
		'upstream_start' : [ # TODO ldprofile
			('a_l',  'rowid',   "(SELECT d_r.posMin {pMinOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMax < a_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_r.posMin {pMinOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMax < m_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_r.posMin {pMinOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMax < d_sl.pos - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
		],
		'upstream_stop' : [ # TODO ldprofile
			('a_l',  'rowid',   "(SELECT d_r.posMax {pMaxOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMax < a_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_r.posMax {pMaxOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMax < m_l.pos  - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_r.posMax {pMaxOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMax < d_sl.pos - {rpMargin} ORDER BY d_r.posMax DESC LIMIT 1)"),
		],
		
		'downstream_id' : [
			('a_l',  'rowid',   "(SELECT d_u.unit_id               FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMin > a_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_u.unit_id               FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMin > m_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_u.unit_id               FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMin > d_sl.pos + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
		],
		'downstream_label' : [
			('a_l',  'rowid',   "(SELECT d_u.label                 FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMin > a_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_u.label                 FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMin > m_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_u.label                 FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMin > d_sl.pos + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
		],
		'downstream_distance' : [
			('a_l',  'rowid',   "-a_l.pos +(SELECT MIN(d_r.posMin) FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMin > a_l.pos  + {rpMargin})"),
			('m_l',  'rowid',   "-m_l.pos +(SELECT MIN(d_r.posMin) FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMin > m_l.pos  + {rpMargin})"),
			('d_sl', '_ROWID_', "-d_sl.pos+(SELECT MIN(d_r.posMin) FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMin > d_sl.pos + {rpMargin})"),
		],
		'downstream_start' : [ # TODO ldprofile
			('a_l',  'rowid',   "(SELECT d_r.posMin {pMinOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMin > a_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_r.posMin {pMinOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMin > m_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_r.posMin {pMinOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMin > d_sl.pos + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
		],
		'downstream_stop' : [ # TODO ldprofile
			('a_l',  'rowid',   "(SELECT d_r.posMax {pMaxOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = a_l.chr  AND d_r.posMin > a_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('m_l',  'rowid',   "(SELECT d_r.posMax {pMaxOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = m_l.chr  AND d_r.posMin > m_l.pos  + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
			('d_sl', '_ROWID_', "(SELECT d_r.posMax {pMaxOffset}   FROM `db`.`unit` AS d_u JOIN `db`.`unit_region` AS d_ur USING (unit_id) JOIN `db`.`region` AS d_r USING (region_id) WHERE d_u.utype_id+0 = {utypeID_gene} AND d_r.chr = d_sl.chr AND d_r.posMin > d_sl.pos + {rpMargin} ORDER BY d_r.posMin LIMIT 1)"),
		],
		...
		'group_id' : [
			...
			('u_gu',   'group_id', "u_gu.group_id"),
			('u_gu_L', 'group_id', "u_gu_L.group_id"),
			('u_gu_R', 'group_id', "u_gu_R.group_id"),
			('u_g',    'group_id', "u_g.group_id"),
			('d_gu',   'group_id', "d_gu.group_id"),
			('d_gu_L', 'group_id', "d_gu_L.group_id"),
			('d_gu_R', 'group_id', "d_gu_R.group_id"),
	...
	def buildQuery(self, mode, focus, select, having=None, where=None, applyOffset=False, fromFilter=None, joinFilter=None, userKnowledge=False):
		...
		for alias,dbtable in self._queryAliasTable.iteritems():
			...
			if (alias == 'c_mu_R') and ((self._options.alternate_model_filtering == 'yes') or fromFilter.get('cand',empty).get('alt_unit')):
				continue
			# otherwise, add it
		...
		options = {
			'L'                  : None, #set dynamically for each set of join conditions
			'R'                  : None, #set dynamically for each set of join conditions
			...
			'utypeID_gene'       : self.getOptionUTypeID('gene', optional=True),
			...
			'guColumn1'          : 'specificity',
			'guColumn2'          : 'specificity',
			'guCondition'        : ('> 0' if self._options.allow_ambiguous_knowledge == 'yes' else '>= 100'),
		}
		#TODO ldprofile
	...
	#		sys.exit("ERROR: %s LD profile record not found in the knowledge database" % (self._options.ld_profile or '<default>',))
		...
		if self._options.reduce_ambiguous_knowledge == 'yes':
			options['guColumn1'] = ('implication' if (self._options.reduce_ambiguous_knowledge == 'any') else self._options.reduce_ambiguous_knowledge)
			options['guColumn2'] = ('quality'     if (self._options.reduce_ambiguous_knowledge == 'any') else self._options.reduce_ambiguous_knowledge)
	...
	def _populateColumnsFromTypes(self, types, columns=None, header=None, ids=None):
		...
		for t in types:
			if t == 'snp':
				...
				columns.extend(['unit_chr','gene_label','unit_start','unit_stop'])
			elif t == 'upstream':
	...
	def identifyCandidateModelUnits(self):
		cursor = self._loki._db.cursor()
		
		# reset candidate tables
		self._inputFilters['cand']['main_unit'] = 0
		self.prepareTableForUpdate('cand','main_unit')
		cursor.execute("DELETE FROM `cand`.`main_unit`")
		self._inputFilters['cand']['alt_unit'] = 0
		cursor.execute("DELETE FROM `cand`.`alt_unit`")
		self.prepareTableForUpdate('cand','alt_unit')
		...
		if sum(filters for table,filters in self._inputFilters['main'].iteritems() if table not in ('group','source')):
			...
			query = self.buildQuery(mode='modelgene', focus='main', select=['gene_id' if self._onlyGeneModels else 'unit_id'])
			sql = "INSERT OR IGNORE INTO `cand`.`main_unit` (unit_id, flag) VALUES (?,0)"
			cursor.executemany(sql, self.generateQueryResults(query, allowDupes=True))
			numCand = max(row[0] for row in cursor.execute("SELECT COUNT() FROM `cand`.`main_unit`"))
			self.log(" OK: %d candidates\n" % numCand)
			self._inputFilters['cand']['main_unit'] = 1
		...
		if sum(filters for table,filters in self._inputFilters['alt'].iteritems() if table not in ('group','source')):
			...
			query = self.buildQuery(mode='modelgene', focus='alt', select=['gene_id' if self._onlyGeneModels else 'unit_id'])
			sql = "INSERT OR IGNORE INTO `cand`.`alt_unit` (unit_id, flag) VALUES (?,0)"
			cursor.executemany(sql, self.generateQueryResults(query, allowDupes=True))
			numCand = max(row[0] for row in cursor.execute("SELECT COUNT() FROM `cand`.`alt_unit`"))
			self.log(" OK: %d candidates\n" % numCand)
			self._inputFilters['cand']['alt_unit'] = 1
		#if any alt filters other than group/source
	#identifyCandidateModelUnits()
	...
	def identifyCandidateModelGroups(self):
		...
		query = self.buildQuery(mode='modelgroup', focus='cand', select=['group_id'], having={('gene_id' if self._onlyGeneModels else 'unit_id'):{'!= 0'}})
		if self._inputFilters['cand']['group']:
			cursor.execute("UPDATE `cand`.`group` SET flag = 0")
			sql = "UPDATE `cand`.`group` SET flag = 1 WHERE group_id = ?"
		else:
		...
		for source in self._queryColumnSources['group_id']:
			if source[0] in query['FROM']:
				...
				break
		for source in self._queryColumnSources['gene_id' if self._onlyGeneModels else 'unit_id']:
			if source[0] in query['FROM']:
	...
	def getGeneModels(self):
		...
		if self._geneModels == None:
			...
			self.identifyCandidateModelUnits()
			self.identifyCandidateModelGroups()
			
			# build model query
			formatter = string.Formatter()
			query = self.buildQuery(mode='model', focus='cand', select=['unit_id_L','unit_id_R','source_id','group_id'])
			query['GROUP BY'].append(formatter.vformat("MIN({unit_id_L}, {unit_id_R})", args=None, kwargs=query['SELECT']))
			query['GROUP BY'].append(formatter.vformat("MAX({unit_id_L}, {unit_id_R})", args=None, kwargs=query['SELECT']))
			query['SELECT']['unit_id_L'] = "MIN(%s)" % query['SELECT']['unit_id_L']
			query['SELECT']['unit_id_R'] = "MAX(%s)" % query['SELECT']['unit_id_R']
	...
	def generateModelOutput(self, typesL, typesR, applyOffset=False):
		...
		if self._options.all_pairwise_models != 'yes':
			conditionsL = {('gene_id' if self._onlyGeneModels else 'unit_id') : {"= (CASE WHEN 1 THEN ?1 ELSE 0*?2*?3*?4 END)"}}
			conditionsR = {('gene_id' if self._onlyGeneModels else 'unit_id') : {"= (CASE WHEN 1 THEN ?2 ELSE 0*?1*?3*?4 END)"}}
