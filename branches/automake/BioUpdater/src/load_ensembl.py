...
class EnsemblLoader(bioloader.BioLoader):
	...
	def ConnectToEnsemblDB(self, host = "rogue", user="torstees", password="SMOJ2010"):
		self.ensembl		= MySQLdb.connect (host, user, password, db="variation")
	...
	def GrabEnsemblVersionDetails(self):
		...
		c.execute("SELECT version FROM ensembl.biodb_versions WHERE element='build'")
		row = c.fetchone()
		self.ncbiVersion = row[0]
		
		c.execute("SELECT version FROM ensembl.biodb_versions WHERE element='enembl'")
		row = c.fetchone()
		self.version     = row[0]
		self.biosettings.SetVersion("build", self.ncbiVersion)
		self.biosettings.SetVersion("ensembl", self.version)
...
FROM 
	variation.variation a INNER JOIN 
	variation.variation_feature b ON (a.variation_id = b.variation_id) INNER JOIN 
	(SELECT * FROM ensembl.seq_region WHERE coord_system_id=%s AND name=%s) c ON (b.seq_region_id = c.seq_region_id)
WHERE a.name LIKE %s""", (self.coordinate, chrom, 'rs%'))
	...
	def WriteChromosome(self, file, chrom, textFile):
		...
		print>> sys.stderr, """SELECT COUNT(a.name), c.name AS chromosome, MAX(seq_region_start) AS position
				FROM variation.variation a
					INNER JOIN variation.variation_feature b ON a.variation_id = b.variation_id
					INNER JOIN ensembl.seq_region c ON b.seq_region_id = c.seq_region_id
				WHERE c.coord_system_id=%s AND a.name LIKE '%s' AND c.name=%s GROUP BY c.name\n\n""" % (self.coordinate, 'rs%', chrom)

		#Table of Contents for the variations
		cursor.execute("""SELECT COUNT(a.name), c.name AS chromosome, MAX(seq_region_start) AS position
				FROM variation.variation a
					INNER JOIN variation.variation_feature b ON a.variation_id = b.variation_id
					INNER JOIN ensembl.seq_region c ON b.seq_region_id = c.seq_region_id
				WHERE c.coord_system_id=%s AND a.name LIKE %s AND c.name=%s GROUP BY c.name""", (self.coordinate, 'rs%', chrom))
		row = cursor.fetchone()
		chr = row[1]
		if (len(chr) > 1):
			file.write(chr[:2])
		...
		cursor.execute("""SELECT a.name as rs_id,seq_region_start AS position, b.consequence_type as role
				FROM variation.variation a
					INNER JOIN variation.variation_feature b ON a.variation_id = b.variation_id
					INNER JOIN (SELECT * FROM ensembl.seq_region WHERE coord_system_id=%s AND name=%s) c ON b.seq_region_id = c.seq_region_id
				WHERE a.name LIKE %s""", (self.coordinate, chrom, 'rs%'))
		...
		while (row):
			...
			file.write(struct.pack('IIc', (int)(row[0][2:]), row[1], '%c' % roleID))
