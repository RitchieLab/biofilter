...
void Main::LoadSNPs() {
	...
	if (snpFilename.size() > 0) {
		...
		std::pair<uint, uint> returns = app.LoadVcfFile(snpFilename, genomicBuild, lostSnps);
		std::cerr<<"VCF Loaded: "<<returns.first<<" bin";
		if (returns.first != 1)
			std::cerr<<"s";
		std::cerr<<" in use and "<<returns.second<<" genotype";
		if (returns.second != 1)
			std::cerr<<"s";
		std::cerr<<" were identified. \n";
