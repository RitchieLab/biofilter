...
$B --plan notebooks/data/bundle_lifecycle/01_core.json --out $OUT

# 2) gnomAD chr1-5: the heavy end. chr1 alone is 126 GB of raw.
$B --plan notebooks/data/bundle_lifecycle/02_gnomad_1_5.json --out $OUT

# 3) gnomAD chr6-13
$B --plan notebooks/data/bundle_lifecycle/03_gnomad_6_13.json --out $OUT

# 4) gnomAD chr14-22, X, Y
$B --plan notebooks/data/bundle_lifecycle/04_gnomad_14_y.json --out $OUT

# 5) AlphaMissense, GTEx, GWAS — genome-wide, not per chromosome
$B --plan notebooks/data/bundle_lifecycle/05_other_variants.json --out $OUT

# 6) Assemble. Everything is already done, so this only builds the bundle.
.venv/bin/biofilter bundle build \
  --plan notebooks/data/bundle_lifecycle/06_assemble.json \
...
biofilter bundle build \
  --plan notebooks/data/bundle_lifecycle/07_chr22_predictors.json \
  --data-root biofilter_data \
