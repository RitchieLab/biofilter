...
uint KbMetaGroup::AssociateGenes(soci::session& sociDB, map<uint,KbRegion*>& genes, SnpManager& snps, map<uint, string>& aliases, uint popID) {
	...
	string sql = "SELECT group_id, regions.gene_id, ensembl_id, chrom, start, end, description FROM (SELECT * FROM region_bounds WHERE population_id=" + ToString(popID) + ") NATURAL JOIN regions INNER JOIN group_associations ON (regions.gene_id=group_associations.gene_id) WHERE group_id IN (" + groupIDs +") ORDER BY group_id";
//cerr<<sql<<"\n";
	...
	for (rowset<row>::const_iterator itr = rs.begin(); itr != rs.end(); ++itr) {
		...
		if (genes.find(gene_id) == genes.end()) {
			...
			string ensembl = row.get<string>(2);
