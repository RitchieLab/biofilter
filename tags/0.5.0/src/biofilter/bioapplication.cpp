...
map<uint, string> BioApplication::LoadRegionAlias(const char *filename) {
	...
	if (filename) {
		...
		rowset<row> rs = (sociDB.prepare << ("SELECT gene_id, alias, alias_desc, region_alias_type_desc, ensembl_id FROM region_alias_type NATURAL JOIN region_alias NATURAL JOIN regions " + aliases));
		for (rowset<row>::const_iterator itr = rs.begin(); itr != rs.end(); ++itr) {
			row const& row = *itr;
...
void BioApplication::LoadRegionAliases(const string& geneList) {
	...
	rowset<row> rs = (sociDB.prepare << "SELECT gene_id, alias, alias_desc, region_alias_type_desc, ensembl_id FROM region_alias_type NATURAL JOIN region_alias NATURAL JOIN regions WHERE region_alias_type_id IN (1300, 2000, 2200) " + geneFilter + " ORDER BY ensembl_id, alias DESC");
	for (rowset<row>::const_iterator itr = rs.begin(); itr != rs.end(); ++itr) {
		row const& row = *itr;
		size_t geneID = row.get<int>(0);
		string alias = row.get<string>(1);	
...
void BioApplication::CleanRSIDs(std::vector<uint>& snpList, const char *rsCleanReportFilename, bool reportCounts) {

	std::set<uint> snps;
	snps.insert(snpList.begin(), snpList.end());
	string commaSnpList = Utility::ToString<set<uint> >(snps, ",");
	uint totalRenamed = 0;				///< Number of actual IDs that were renamed
	string sql = "SELECT merged_rs_id, current_rs_id, expired FROM rs_merged WHERE merged_rs_id in (" + commaSnpList + ");";
	...
	try {
		...
		if (expiredRS.size() > 0 || renamedRS.size() > 0) {
			...
			string exSS = "\trs" + Utility::ToString<set<uint> >(expiredRS, "\n\trs");
			file<<exSS<<"\n"<<totalRenamed<<" rs IDs were updated\n\tOriginal ID\tNew ID\n"<<ss.str()<<"\n";
		}
	} catch (exception const &e) {
		std::vector<uint>::iterator itr = snpList.begin();
		std::vector<uint>::iterator end = snpList.end();

		while (itr != end) {
			cerr<<*itr++<<"\n";
		}
		cerr<<"\nThere were: "<<snps.size()<<" snps there.\n"<<commaSnpList<<"\n";
...
void BioApplication::InitBiofilter(const char *dbFilename, bool reportVersion) {
	...
	try {
		string cnxParam = "dbname="+string(dbFilename)+" timeout=25";
		...
		if (reportVersion) {
			...
			cerr<<setw(35)<<right<<"Ensembl: "<<ensmbl<<"\n";
