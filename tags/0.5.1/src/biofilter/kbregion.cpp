...
KbRegion::KbRegion() : KbEntity(0, "", ""),Region("", 0),
	startPosition(0), endPosition(0), chromosome(""), snpLookup(NULL) { }
...
KbRegion::KbRegion(uint geneID, uint start, uint end, const char *chrom, const char *ensembl, const char *desc, SnpManager* snpLookup)
	: KbEntity(geneID, ensembl, desc), Region(ensembl, geneID), startPosition(start), endPosition(end), chromosome(chrom), snpLookup(snpLookup) { }

KbRegion::KbRegion(const KbRegion& other):
	KbEntity(other), Region(other), startPosition(other.startPosition), endPosition(other.endPosition), chromosome(other.chromosome), snpLookup(other.snpLookup) { }
