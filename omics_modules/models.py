...
class DataSource(Base):
    ...
    )  # Versão do Data Transformation Process
    last_update = Column(
...
DataSource.workprocesses = relationship(
    "WorkProcess", back_populates="data_source", cascade="all, delete-orphan"
)
...
class ExtractedFile(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    workprocess_id = Column(
        Integer, ForeignKey("workprocesses.id", ondelete="CASCADE"), nullable=False
    )
...
WorkProcess.extracted_files = relationship(
    "ExtractedFile", back_populates="workprocess", cascade="all, delete-orphan"
)
...
class Assembly(Base):
    ...
    source = Column(
        String, nullable=False
    )  # Source of the assembly (e.g., GENCODE, UCSC, Ensembl)
    active = Column(
        Boolean, default=True
    )  # Indicates if the assembly is currently in use
    created_at = Column(
        DateTime, default=lambda: datetime.datetime.now(datetime.timezone.utc)
    )
    updated_at = Column(
...
class OmicsRelationship(Base):
    ...
    entity_1_type = Column(
        String, nullable=False
    )  # Tipo da primeira entidade (Gene, SNP, Protein, etc.)
    ...
    relationship_type = Column(
        String, nullable=False
    )  # "is_a", "part_of", "regulates", "has_part"
...
class OmicsTermAlternative(Base):
    ...
    )  # ID da entidade (Gene, SNP, Metabolito, etc.)
    ...
    term_type = Column(
        String, nullable=False
    )  # Tipo de relacionamento (official, alias, previous, synonym)
