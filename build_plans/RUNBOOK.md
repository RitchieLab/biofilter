...
$B --plan build_plans/01_core.json --out $OUT

# 2) gnomAD chr1-5: the heavy end. chr1 alone is 126 GB of raw.
$B --plan build_plans/02_gnomad_1_5.json --out $OUT

# 3) gnomAD chr6-13
$B --plan build_plans/03_gnomad_6_13.json --out $OUT

# 4) gnomAD chr14-22, X, Y
$B --plan build_plans/04_gnomad_14_y.json --out $OUT

# 5) AlphaMissense, GTEx, GWAS — genome-wide, not per chromosome
$B --plan build_plans/05_other_variants.json --out $OUT

# 6) Assemble. Everything is already done, so this only builds the bundle.
.venv/bin/biofilter bundle build \
  --plan build_plans/06_assemble.json \
