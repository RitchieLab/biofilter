...
def snp_input_mixin():
    ...
    mixin._loki._biofilter.db.cursor.return_value = MagicMock()
...
def test_unionInputSNPs(snp_input_mixin):
    ...
    snp_input_mixin._loki._biofilter.db.cursor().executemany.assert_called_once()
...
def test_intersectInputSNPs(snp_input_mixin):
    ...
    snp_input_mixin._loki._biofilter.db.cursor().getconnection.return_value.changes.return_value = (  # noqa  E501
    ...
    snp_input_mixin._loki._biofilter.db.cursor().execute.assert_has_calls(
        [
            call("UPDATE `%s`.`snp` SET flag = 0" % db),
            call("DELETE FROM `%s`.`snp` WHERE flag = 0" % db),
        ]
    )
    snp_input_mixin._loki._biofilter.db.cursor().executemany.assert_called_once_with(
