...
class Updater(object):
        ...
        if self._sourceLoaders == None:
            ...
            for srcImporter, srcModuleName, _ in pkgutil.iter_modules(loader_path):
                if srcModuleName.startswith("loki_source_"):
                    self._sourceLoaders[srcModuleName[12:]] = srcImporter.find_module(
                        srcModuleName
                    )
    ...
    def loadSourceModules(self, sources=None):
        ...
        for srcName in set(sources) if sources else self._sourceLoaders.keys():
            if srcName not in self._sourceClasses:
                ...
                srcModule = self._sourceLoaders[srcName].load_module(
                    "loki_source_%s" % srcName
                )
    ...
    def getSourceModuleOptions(self, sources=None):
        srcSet = self.loadSourceModules(sources)
        return {
            srcName: self._sourceClasses[srcName].getOptions() for srcName in srcSet
        }
    ...
    def attachSourceModules(self, sources=None):
        ...
        for srcName in sources:
            if srcName not in self._sourceObjects:
                if srcName not in self._sourceClasses:
                    raise Exception(
                        "loadSourceModules() reported false positive for '%s'" % srcName
                    )
        ...
        try:
                ...
                try:
                    ...
                    if skip:
                        ...
                        for row in cursor.execute(
                            ...
                            skip = (
                                skip
                                and (row[0] in filehash)
                                and (row[1] == filehash[row[0]][1])
                                and (row[2] == filehash[row[0]][3])
                            )
                    ...
                    if skip:
                        ...
                        cursor.execute(
                            "DELETE FROM `db`.`warning` WHERE source_id = ?", (srcID,)
                        )
                        ...
                        sql = (
                            "INSERT INTO `db`.`source_option` (source_id, option, value) VALUES (%d,?,?)"
                            % srcID
                        )
                        ...
                        sql = (
                            "INSERT INTO `db`.`source_file` (source_id, filename, size, modified, md5) VALUES (%d,?,?,DATETIME(?,'unixepoch'),?)"
                            % srcID
                        )
                ...
                for tablerow in re.finditer(
                    ...
                    cols = tuple(
                        ...
                        for match in re.finditer(
                            r"<td>.*?</td>", tablerow.group(), re.IGNORECASE | re.DOTALL
                        )
                    )
                    if cols and (
                        (cols[0] == "human")
            ...
            if len(hgSources) > 1:
                ...
                for oldHG in sorted(hgSources):
                    ...
                    sql = (
                        "UPDATE `db`.`source` SET current_ucschg = %d WHERE source_id = ?"
                        % targetHG
                    )
                    cursor.executemany(
    ...
    def liftOverSNPLoci(self, oldHG, newHG, sourceIDs):
        ...
        firstRowID = min(
            row[0]
            for row in cursor.execute("SELECT MIN(_ROWID_) FROM `db`.`snp_locus`")
        )
        lastRowID = max(
            row[0]
            for row in cursor.execute("SELECT MAX(_ROWID_) FROM `db`.`snp_locus`")
        )
    ...
    def liftOverRegions(self, oldHG, newHG, sourceIDs):
        ...
        while minRowID <= lastRowID:
            ...
            if trash:
                cursor.executemany(
                    "DELETE FROM `db`.`biopolymer_region` WHERE _ROWID_ = ?", trash
                )
                trash.clear()
            minRowID = maxRowID + 1
            maxRowID = minRowID + 2500000 - 1
...
"""
    ...
    def cleanupSNPLoci(self):
        ...
        if valid:
            dbc.executemany(
                "UPDATE `db`.`snp_locus` SET validated = 1 WHERE _ROWID_ = ?", valid
            )
...
"""
        ...
        self.log(
            " OK: %d identifiers (%d ambiguous, %d unrecognized)\n"
...
""",
        ...
        if cull:
            ...
            dbc.executemany(
                "DELETE FROM `db`.`snp_biopolymer_role` WHERE _ROWID_ = ?", cull
            )

        numTotal = numSNPs = numGenes = 0
        for row in dbc.execute(
            "SELECT COUNT(), COUNT(DISTINCT rs), COUNT(DISTINCT biopolymer_id) FROM `db`.`snp_biopolymer_role`"
        ):
            numTotal = row[0]
            numSNPs = row[1]
            numGenes = row[2]
        self.log(
            " OK: %d roles (%d SNPs, %d genes; %d unrecognized)\n"
...
"""
        ...
        dbc.execute(
            "CREATE INDEX `temp`.`_group_member_name_score__group_member_biopolymer` ON `_group_member_name_score` (group_id, member, biopolymer_id)"
        )
...
"""
            ...
            numUnrec = row[4]
        self.log(
            " OK: %d associations (%d explicit, %d definite, %d conditional, %d unrecognized)\n"
    ...
    def updateBiopolymerZones(self):
        ...
        dbc.execute(
            "UPDATE `db`.`biopolymer_region` SET posMin = posMax, posMax = posMin WHERE posMin > posMax"
        )
        ...
        for row in dbc.execute(
            "SELECT COUNT(), COUNT(DISTINCT biopolymer_id) FROM `db`.`biopolymer_zone`"
        ):
            numTotal = row[0]
            numGenes = row[1]
