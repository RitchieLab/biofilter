...
"""
			...
			for line in mergeFile:
				...
				if len(setMerge) >= 2500000:
					...
					self.log("processing SNP merge records: ~%1.1f million so far\n" % (numMerge/1000000.0)) #TODO: time estimate
			...
			numMerge += len(setMerge)
			self.log("processing SNP merge records completed: ~%d merged RS#s\n" % numMerge)
			if setMerge:
				self.log("writing SNP merge records to the database ...\n")
				self.addSNPMerges(setMerge)
				self.log("writing SNP merge records to the database completed\n")
...
"""
			...
			for line in funcFile:
				...
				if len(setRole) >= 2500000:
					numRole += len(setRole)
					self.log("processing SNP roles: ~%1.1f million so far\n" % (numRole/1000000.0)) #TODO: time estimate
		...
		for fileChm in self._chmList:
			processChmThreads[fileChm] = Thread(target=self.processChmSNPs, args=(fileChm, reBuild, includeUnvalidated, includeSuspect, includeWithdrawn, path))
	...
	def processChmSNPs(self, fileChm, reBuild, includeUnvalidated, includeSuspect, includeWithdrawn, path):
			...
			if rs != '' and chm != '' and pos != '':
				...
				if not build:
					...
					if numPosBatch >= 2500000:
						numPos += numPosBatch
						numPosBatch = 0
						self.log("processing chromosome %s: %1.1f million so far\n" % (fileChm, numPos/1000000.0))
						# store data
